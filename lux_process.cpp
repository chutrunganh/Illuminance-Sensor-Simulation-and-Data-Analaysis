...
void min_max_adverage( int argc, char* argv[]) {
    ...
    vector<int> locationMap(15,0); //locationMap[id] = location, maximum 15 id, default all value = 0
    ...
    while (getline(locationFile, line)) {
        ...
        getline(ss, line, ',');
        if(line.empty()){
            writelog("Error 05 : unknown location of sensor ID"); //if location code is empty
            continue;
        }
        location = stoi(line);
    ...
    for (int i = 0; i < numberOfConditionLines; i++) {
        ...
        double adverage_value = id_per_hour_adverage_value[line_components[i].id][line_components[i].hour];
        string condition_output = table1(locationMap[line_components[i].id],adverage_value);
        conditionFile << line_components[i].id << ',' << line_components[i].timpestamp.substr(0,10) << " " << line_components[i].hour << ":00:00" << ',' << locationMap[line_components[i].id] << ',' 
